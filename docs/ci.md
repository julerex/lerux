...
4. **smoke** — 40 parallel matrix jobs; each restores SDK artifact, per-job `build/` cache, and SP804 QEMU (init/composed/blk-composed/http-composed/net-composed/ipc-composed/workstation only; init-riscv/init-x86 use stock QEMU). Serial captures land in `build/smoke-logs/` and upload as `smoke-serial-<id>` (Phase 57).
...
| `agent` | `just test-agent` | Phase 79: FS tools + WebFetch (`lerux-agent: tools ok`) |
...
| ipc-composed times out on `virtio-net: MAC` | Concurrent `debug_putchar` splices mid-line (`virtio-` / `net: MAC`). Smoke matching reconstructs unfinished `INFO [target]` lines as a FIFO so nested splices (and an earlier split prefix) do not steal leftovers |
| isolation times out on `lerux-debug: ready` | `INFO  [serial_driver]INFO  [debug_handler]` with serial's payload on the debug line. Matcher treats `]INFO` as a prefix boundary and pairs the first newline with the preempted prefix-only line |
