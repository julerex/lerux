...
/// A [`HashMap`](std::collections::HashMap) using [`RandomState`](crate::ahash::RandomState) to hash the items.
/// (Requires the `std` feature to be enabled.)
#[derive(Clone)]
pub struct AHashMap<K, V, S = crate::ahash::RandomState>(HashMap<K, V, S>);

impl<K, V> From<HashMap<K, V, crate::ahash::RandomState>> for AHashMap<K, V> {
    fn from(item: HashMap<K, V, crate::ahash::RandomState>) -> Self {
        AHashMap(item)
    }
}
...
impl<K, V> Into<HashMap<K, V, crate::ahash::RandomState>> for AHashMap<K, V> {
    fn into(self) -> HashMap<K, V, crate::ahash::RandomState> {
        self.0
    }
}
