...
// (cfg_attr doc injection above removed for inlined no_std kernel build; it only activates under std+rng features we do not use.)
...
cfg_if! {
    if #[cfg(any(
        ...
        mod aes_hash;
        pub use crate::ahash::aes_hash::AHasher;
    } else {
        pub use crate::ahash::fallback_hash::AHasher;
    }
}
...
cfg_if! {
    if #[cfg(feature = "std")] {
        ...
        pub use crate::ahash::hash_map::AHashMap;
        pub use crate::ahash::hash_set::AHashSet;

        /// [Hasher]: std::hash::Hasher
        /// [HashMap]: std::collections::HashMap
        /// Type alias for [HashMap]<K, V, ahash::RandomState>
        pub type HashMap<K, V> = std::collections::HashMap<K, V, crate::ahash::RandomState>;

        /// Type alias for [HashSet]<K, ahash::RandomState>
        pub type HashSet<K> = std::collections::HashSet<K, crate::ahash::RandomState>;
    }
}
...
pub use crate::ahash::random_state::RandomState;
