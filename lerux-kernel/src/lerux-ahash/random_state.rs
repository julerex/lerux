...
// (simplified for lerux zero-dep inlined no_std build: no once_cell, always use direct static DefaultRandomSource
//  so gen_hasher_seed is always available. set_random_source is stubbed as it is not used in kernel.)
...
impl RandomSource for DefaultRandomSource {
    // Always provide the simple impl (simplified for lerux inlined kernel; no arm special case needed for x86_64 etc.)
    fn gen_hasher_seed(&self) -> usize {
...
// Simple static default for all targets in kernel (no once_cell, no set support needed).
#[inline]
fn get_src() -> &'static dyn RandomSource {
    static RAND_SOURCE: DefaultRandomSource = DefaultRandomSource {
        counter: AtomicUsize::new(0),
    };
    &RAND_SOURCE
}

// Stub: kernel does not call set_random_source; keep symbol if referenced by RandomState docs or reexports but make it unavailable at runtime use.
#[cfg(not(all(target_arch = "arm", target_os = "none")))]
#[allow(dead_code)]
pub fn set_random_source(_source: impl RandomSource + Send + Sync + 'static) -> Result<(), bool> {
    Err(false)
}
