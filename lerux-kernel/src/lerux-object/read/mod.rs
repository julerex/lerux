...
pub type NativeFile<'data, R = &'data [u8]> =
    macho::MachOFile32<'data, crate::object::endian::Endianness, R>;

/// The native executable file for the target platform.
#[cfg(all(target_os = "macos", target_pointer_width = "64", feature = "macho"))]
pub type NativeFile<'data, R = &'data [u8]> =
    macho::MachOFile64<'data, crate::object::endian::Endianness, R>;
        ...
        let kind = match [magic[0], magic[1], magic[2], magic[3], magic[4], magic[5], magic[6], magic[7]] {
            ...
            [b'M', b'Z', ..] if offset == 0 => {
                ...
                match pe::optional_header_magic(data) {
                    Ok(crate::object::pe::IMAGE_NT_OPTIONAL_HDR32_MAGIC) => {
                        FileKind::Pe32
                    }
                    Ok(crate::object::pe::IMAGE_NT_OPTIONAL_HDR64_MAGIC) => {
                        FileKind::Pe64
                    }
            ...
            [0x00, 0x00, 0xff, 0xff, 0x02, 0x00, ..] if offset == 0 => {
                ...
                match coff::anon_object_class_id(data) {
                    Ok(crate::object::pe::ANON_OBJECT_HEADER_BIGOBJ_CLASS_ID) => FileKind::CoffBig,
