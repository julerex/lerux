...
// (bitflags inlined at root with #[macro_use]; extern removed for zero-dep inlining)
...
pub mod native_cpuid {
    use crate::raw_cpuid::CpuIdResult;
...
macro_rules! cpuid {
    ($eax:expr) => {
        $crate::raw_cpuid::native_cpuid::cpuid_count($eax as u32, 0)
    };

    ($eax:expr, $ecx:expr) => {
        $crate::raw_cpuid::native_cpuid::cpuid_count($eax as u32, $ecx as u32)
    };
}
...
/// (see [CACHE_INFO_TABLE](crate::raw_cpuid::CACHE_INFO_TABLE)).
///
/// # Platforms
/// ❌ AMD ✅ Intel
...
impl Debug for FeatureInfo {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("FeatureInfo")
            ...
            .field("edx_ecx", &self.edx_ecx.bits())
            .finish()
    }
}
...
impl Debug for ExtendedFeatures {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("ExtendedFeatures")
            .field("ebx", &self.ebx.bits())
            .field("ecx", &self.ecx.bits())
...
impl Debug for ExtendedStateInfo {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("ExtendedStateInfo")
            .field("eax", &self.eax.bits())
            .field("ecx1", &self.ecx1.bits())
            .field(
