...
macro_rules! smallvec {
    ...
    ($elem:expr; $n:expr) => ({
        $crate::smallvec::SmallVec::from_elem($elem, $n)
    });
    ($($x:expr),+$(,)?) => ({
        let count = 0usize $(+ $crate::smallvec::smallvec!(@one $x))+;
        let mut vec = $crate::smallvec::SmallVec::new();
        if count <= vec.inline_size() {
            $(vec.push($x);)*
            vec
        } else {
            $crate::smallvec::SmallVec::from_vec($crate::smallvec::alloc::vec![$($x,)+])
        }
    });
}
...
macro_rules! smallvec_inline {
    ...
    (@one $x:expr) => (1usize);
    ($elem:expr; $n:expr) => ({
        $crate::smallvec::SmallVec::<[_; $n]>::from_const([$elem; $n])
    });
    ($($x:expr),+ $(,)?) => ({
        const N: usize = 0usize $(+ $crate::smallvec::smallvec_inline!(@one $x))*;
        $crate::smallvec::SmallVec::<[_; N]>::from_const([$($x,)*])
    });
}
