...
// Inlined crate aliases (zero-dep kernel build).
use crate::lock_api as lock_api_crate;

// Always enable spin mutex for the inlined kernel build (no Cargo feature flags).
pub mod barrier;
pub mod lazy;
pub mod mutex;
pub mod once;
...
pub use relax::Spin as Yield; // (Yield is std-only; alias Spin for inlined kernel)
...
// Force aliases for the items the kernel arch code expects (in case cfg stripping removed the lines).
