...
/// }
/// ```
pub type Spinlock<T> = crate::lock_api::Mutex<RawSpinlock, T>;
...
/// ```
pub type SpinlockGuard<'a, T> = crate::lock_api::MutexGuard<'a, RawSpinlock, T>;
...
/// ```
pub type MappedSpinlockGuard<'a, T> = crate::lock_api::MappedMutexGuard<'a, RawSpinlock, T>;
...
pub const fn const_spinlock<T>(val: T) -> Spinlock<T> {
    Spinlock::const_new(<RawSpinlock as crate::lock_api::RawMutex>::INIT, val)
}
