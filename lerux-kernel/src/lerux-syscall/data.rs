...
use super::flag::{EventFlags, MapFlags, PtraceFlags, StdFsCallKind};
...
macro_rules! ptrace_event {
    ($cause:expr $(, $a:expr $(, $b:expr $(, $c:expr)?)?)?) => {
        $crate::__redox_syscall::data::PtraceEvent {
            cause: $cause,
            $(a: $a,
              $(b: $b,
                $(c: $c,)?
...
pub use crate::__redox_syscall::sigabi::*;
...
pub struct CtxtStsBuf {
    pub status: usize,
    pub excp: crate::__redox_syscall::Exception,
}
