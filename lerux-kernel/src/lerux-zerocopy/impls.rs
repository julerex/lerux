...
use super::*;
use crate::zerocopy::pointer::cast::{CastSizedExact, CastUnsized};
...
mod atomics {
    ...
    macro_rules! unsafe_impl_transmute_from_for_atomic {
        ($($($tyvar:ident)? => $atomic:ty [$prim:ty]),*) => {{
            crate::zerocopy::util::macros::__unsafe();

            use crate::zerocopy::pointer::{SizeEq, TransmuteFrom, invariant::Valid};
            ...
            $(
                ...
                impl<$($tyvar)?> SizeEq<ReadOnly<$atomic>> for ReadOnly<$prim> {
                    type CastFrom = $crate::zerocopy::pointer::cast::CastSizedExact;
                }
...
const _: () = {
    ...
    unsafe impl<T: ?Sized> HasTag for ManuallyDrop<T> {
        ...
        type ProjectToTag = crate::zerocopy::pointer::cast::CastToUnit;
    }
    ...
    unsafe impl<T: ?Sized>
        HasField<value, { crate::zerocopy::STRUCT_VARIANT_ID }, { crate::zerocopy::ident_id!(value) }>
        for ManuallyDrop<T>
    {
        #[inline]
        fn only_derive_is_allowed_to_implement_this_trait()
...
const _: () = unsafe {
    ...
    unsafe_impl!(const N: usize, T: TryFromBytes => TryFromBytes for [T; N]; |c| {
        let c: Ptr<'_, [ReadOnly<T>; N], _> = c.cast::<_, crate::zerocopy::pointer::cast::CastSized, _>();
        let c: Ptr<'_, [ReadOnly<T>], _> = c.as_slice();
        let c: Ptr<'_, ReadOnly<[T]>, _> = c.cast::<_, crate::zerocopy::pointer::cast::CastUnsized, _>();
    ...
    macro_rules! impl_tuple {
        ...
        (@all [$($head_T:ident $head_I:tt)*] [$next_T:ident $next_I:tt $($tail:tt)*]) => {
            ...
            unsafe_impl!($($head_T: TryFromBytes,)* $next_T: TryFromBytes => TryFromBytes for ($($head_T,)* $next_T,); |c| {
                let mut c = c;
                $(TryFromBytes::is_bit_valid(into_inner!(c.reborrow().project::<_, { crate::zerocopy::STRUCT_VARIANT_ID }, { crate::zerocopy::ident_id!($head_I) }>())) &&)*
                    TryFromBytes::is_bit_valid(into_inner!(c.reborrow().project::<_, { crate::zerocopy::STRUCT_VARIANT_ID }, { crate::zerocopy::ident_id!($next_I) }>()))
            });
            ...
            // SAFETY: See safety comment on `ProjectToTag`.
            unsafe impl<$($head_T,)* $next_T> crate::zerocopy::HasTag for ($($head_T,)* $next_T,) {
                #[inline]
                fn only_derive_is_allowed_to_implement_this_trait()
                where
                    Self: Sized
        ...
        (@variants
            ...
            unsafe impl<$($AllT),+> crate::zerocopy::HasField<
                (),
                { crate::zerocopy::STRUCT_VARIANT_ID },
                { crate::zerocopy::ident_id!($CurrI)}
            > for ($($AllT,)+) {
                #[inline]
                fn only_derive_is_allowed_to_implement_this_trait()
                where
                ...
                #[inline(always)]
                fn project(slf: crate::zerocopy::PtrInner<'_, Self>) -> *mut Self::Type {
            ...
            unsafe impl<Aliasing, Alignment, $($AllT),+> crate::zerocopy::ProjectField<
                (),
                (Aliasing, Alignment, crate::zerocopy::invariant::Uninit),
                { crate::zerocopy::STRUCT_VARIANT_ID },
                { crate::zerocopy::ident_id!($CurrI)}
            > for ($($AllT,)+)
            where
                Aliasing: crate::zerocopy::invariant::Aliasing,
                Alignment: crate::zerocopy::invariant::Alignment,
            {
                #[inline]
                fn only_derive_is_allowed_to_implement_this_trait()
                where
                ...
                type Invariants = (Aliasing, Alignment, crate::zerocopy::invariant::Uninit);
            ...
            unsafe impl<Aliasing, Alignment, $($AllT),+> crate::zerocopy::ProjectField<
                (),
                (Aliasing, Alignment, crate::zerocopy::invariant::Initialized),
                { crate::zerocopy::STRUCT_VARIANT_ID },
                { crate::zerocopy::ident_id!($CurrI)}
            > for ($($AllT,)+)
            where
                ...
                Alignment: crate::zerocopy::invariant::Alignment,
            {
                ...
                // well-aligned, so projection preserves both the alignment and
                // validity invariants of the outer pointer.
                type Invariants = (Aliasing, Alignment, crate::zerocopy::invariant::Initialized);
            ...
            unsafe impl<Aliasing, Alignment, $($AllT),+> crate::zerocopy::ProjectField<
                (),
                (Aliasing, Alignment, crate::zerocopy::invariant::Valid),
                { crate::zerocopy::STRUCT_VARIANT_ID },
                { crate::zerocopy::ident_id!($CurrI)}
            > for ($($AllT,)+)
            where
                ...
                Alignment: crate::zerocopy::invariant::Alignment,
            {
                ...
                // well-aligned, so projection preserves both the alignment and
                // validity invariants of the outer pointer.
                type Invariants = (Aliasing, Alignment, crate::zerocopy::invariant::Valid);
