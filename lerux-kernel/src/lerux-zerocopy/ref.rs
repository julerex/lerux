    ...
    T: KnownLayout + Immutable + ?Sized,
{
    ...
    /// [t-unaligned]: crate::zerocopy::Unaligned
    ...
    T: FromBytes + KnownLayout + Immutable + ?Sized,
{
    ...
    pub fn into_ref(r: Self) -> &'a T {
        ...
        if let crate::zerocopy::layout::SizeInfo::Sized { .. } = T::LAYOUT.size_info {
...
where
    ...
    A: crate::zerocopy::invariant::Aliasing,
    [u8]: MutationCompatible<T, A, Initialized, Initialized, R>,
    T: TransmuteFromPtr<T, A, Initialized, Valid, crate::zerocopy::pointer::cast::IdCast, S>,
{
    use crate::zerocopy::pointer::cast::{Cast, Project};
    ...
    unsafe impl<T: ?Sized + KnownLayout> Project<[u8], T> for CastForSized {
        ...
        fn project(src: PtrInner<'_, [u8]>) -> *mut T {
            T::raw_from_ptr_len(
                ...
                <T::PointerMetadata as crate::zerocopy::PointerMetadata>::from_elem_count(0),
            )
            .as_ptr()
        }
    }
...
mod tests {
    ...
    fn test_into_ref_mut() {
        #[allow(unused)]
        use crate::zerocopy::util::AsAddress as _;
