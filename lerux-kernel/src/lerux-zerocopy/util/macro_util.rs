    ...
    ($ty:ty, $trailing_field_name:tt) => {{
        let min_size = {
            let zero_elems: *const [()] =
                $crate::zerocopy::util::macro_util::core_reexport::ptr::slice_from_raw_parts(
                    $crate::zerocopy::util::macro_util::core_reexport::ptr::NonNull::<()>::dangling()
                        .as_ptr()
                        .cast_const(),
                    0,
                );
            ...
            unsafe {
                #[allow(clippy::as_conversions)]
                $crate::zerocopy::util::macro_util::core_reexport::mem::size_of_val_raw(
        ...
        let field = unsafe {
            $crate::zerocopy::util::macro_util::core_reexport::ptr::addr_of!((*ptr).$trailing_field_name)
        };
        // SAFETY:
...
macro_rules! struct_padding {
    ($t:ty, $_align:expr, $_packed:expr, [$($ts:ty),*]) => {{
        ...
        $crate::zerocopy::util::macro_util::size_of::<$t>() - (0 $(+ $crate::zerocopy::util::macro_util::size_of::<$ts>())*)
    }};
}
...
macro_rules! repr_c_struct_has_padding {
    ($t:ty, $align:expr, $packed:expr, [$($ts:tt),*]) => {{
        let layout = $crate::zerocopy::DstLayout::for_repr_c_struct(
            $align,
            $packed,
            &[$($crate::zerocopy::repr_c_struct_has_padding!(@field $ts),)*]
        );
        layout.requires_static_padding() || layout.requires_dynamic_padding()
    }};
    (@field ([$t:ty])) => {
        <[$t] as $crate::zerocopy::KnownLayout>::LAYOUT
    };
    (@field ($t:ty)) => {
        $crate::zerocopy::DstLayout::for_unpadded_type::<$t>()
    };
    (@field [$t:ty]) => {
        <[$t] as $crate::zerocopy::KnownLayout>::LAYOUT
    };
...
macro_rules! union_padding {
    ($t:ty, $_align:expr, $_packed:expr, [$($ts:ty),*]) => {{
        ...
        let mut max = 0;
        $({
            let padding = $crate::zerocopy::util::macro_util::size_of::<$t>() - $crate::zerocopy::util::macro_util::size_of::<$ts>();
            if padding > max {
                max = padding;
            }
        })*
...
macro_rules! enum_padding {
    ($t:ty, $_align:expr, $packed:expr, $disc:ty, $([$($ts:ty),*]),*) => {{
        ...
        let mut max = 0;
        $({
            let padding = $crate::zerocopy::util::macro_util::size_of::<$t>()
                - (
                    $crate::zerocopy::util::macro_util::size_of::<$disc>()
                    $(+ $crate::zerocopy::util::macro_util::size_of::<$ts>())*
                );
            if padding > max {
                max = padding;
            }
...
macro_rules! into_inner {
    ($e:expr) => {
        match $e {
            $crate::zerocopy::util::macro_util::core_reexport::result::Result::Ok(e) => e,
            $crate::zerocopy::util::macro_util::core_reexport::result::Result::Err(i) => match i {},
        }
    };
}
...
macro_rules! ident_id {
    ($field:ident) => {
        $crate::zerocopy::util::macro_util::hash_name(stringify!($field))
    };
    ($field:literal) => {
        $field
    };
    ...
    Dst: TryFromBytes,
{
    ...
    let mu_dst: mem::MaybeUninit<ReadOnly<Dst>> =
        unsafe { crate::zerocopy::util::transmute_unchecked(mu_src) };
    ...
    if Dst::is_bit_valid(ptr.cast::<_, CastSized, _>()) {
        ...
        // SAFETY: `MaybeUninit` has no validity requirements.
        let mu_src: mem::MaybeUninit<Src> = unsafe { crate::zerocopy::util::transmute_unchecked(mu_dst) };
    ...
    Dst: TryFromBytes + Immutable + KnownLayout + ?Sized,
{
    ...
    > {
        ...
        let res = ptr.try_with(#[inline(always)] |ptr| {
            ...
            let ptr = ptr.cast::<_, crate::zerocopy::layout::CastFrom<Dst>, _>();
            ptr.try_into_valid()
        });
        match res {
            Ok(ptr) => {
...
impl<'a, Src, Dst> Wrap<&'a Src, &'a Dst> {
        ...
        Dst: TryFromBytes + Immutable,
    {
        ...
        let src: &Wrapping<Src> =
            unsafe { crate::zerocopy::util::transmute_ref::<_, _, BecauseImmutable>(self.0) };
        ...
        .map(
            ...
            // the same alignment.
            #[inline(always)]
            |dst| unsafe { crate::zerocopy::util::transmute_ref::<_, _, BecauseImmutable>(dst) },
        )
        .map_err(
            #[inline(always)]
            |err| {
                ...
                ValidityError::new(unsafe {
                    crate::zerocopy::util::transmute_ref::<_, _, BecauseImmutable>(err.into_src())
                })
            },
        )
    }
...
impl<'a, Src, Dst> Wrap<&'a mut Src, &'a mut Dst> {
        ...
        Dst: TryFromBytes + IntoBytes,
    {
        ...
        let src: &mut Wrapping<Src> =
            unsafe { crate::zerocopy::util::transmute_mut::<_, _, (_, (_, _))>(self.0) };
        let src = Wrap::new(src);
        <Wrap<&'a mut Wrapping<Src>, &'a mut Wrapping<Dst>> as TryTransmuteMutDst<'a>>
            ...
            // same alignment.
            .map(|dst| unsafe { crate::zerocopy::util::transmute_mut::<_, _, (_, (_, _))>(dst) })
            .map_err(|err| {
                // SAFETY: By the preceding assert, `Src` and `Wrapping<Src>` have the
                // same alignment.
                ValidityError::new(unsafe {
                    crate::zerocopy::util::transmute_mut::<_, _, (_, (_, _))>(err.into_src())
                })
            })
    }
}
    ...
    Dst: KnownLayout<PointerMetadata = usize> + FromBytes + Immutable,
{
    ...
    fn transmute_ref(self) -> &'a Dst {
        let ptr = Ptr::from_ref(self.0)
            ...
            .transmute_with::<Dst, Initialized, crate::zerocopy::layout::CastFrom<Dst>, (crate::zerocopy::pointer::BecauseMutationCompatible, _)>()
    ...
    Dst: KnownLayout<PointerMetadata = usize> + FromBytes + IntoBytes,
{
    ...
    fn transmute_mut(self) -> &'a mut Dst {
        let ptr = Ptr::from_mut(self.0)
            ...
            .transmute_with::<Dst, Initialized, crate::zerocopy::layout::CastFrom<Dst>, _>()
