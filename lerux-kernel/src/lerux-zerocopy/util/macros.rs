...
macro_rules! impl_for_transmute_from {
    (
        ...
        const _: () = {
            ...
            unsafe impl<$($tyvar $(: $(? $optbound +)* $($bound +)*)?)?> $trait for $ty {
                ...
                #[cfg_attr(all(coverage_nightly, __ZEROCOPY_INTERNAL_USE_ONLY_NIGHTLY_FEATURES_IN_TESTS), coverage(off))]
                fn only_derive_is_allowed_to_implement_this_trait() {
                    use crate::zerocopy::pointer::{*, invariant::Valid};
    ...
    (
        ...
        #[inline(always)]
        fn is_bit_valid<Alignment>(candidate: $crate::zerocopy::Maybe<'_, Self, Alignment>) -> bool
        where
            Alignment: $crate::zerocopy::invariant::Alignment,
        {
    ...
    (@inner $(const $constvar:ident : $constty:ty)? , $($tyvar:ident $(: ?$optbound:ident)?)? => $(#[$attrs:meta])* $ty:ty) => {
        const _: () = {
            ...
            unsafe impl<$($tyvar $(: ?$optbound)?)? $(, const $constvar : $constty)?> KnownLayout for $ty {
                ...
                const LAYOUT: crate::zerocopy::DstLayout = crate::zerocopy::DstLayout::for_type::<$ty>();
...
macro_rules! static_assert_dst_is_not_zst {
    ($tyvar:ident) => {{
        use crate::zerocopy::KnownLayout;
        static_assert!($tyvar: ?Sized + KnownLayout => {
            let dst_is_zst = match $tyvar::LAYOUT.size_info {
                crate::zerocopy::SizeInfo::Sized { .. } => false,
                crate::zerocopy::SizeInfo::SliceDst(TrailingSliceLayout { elem_size, .. }) => {
                    elem_size == 0
                }
            };
            !dst_is_zst
...
///
/// [`Cast`]: crate::zerocopy::pointer::cast::Cast
#[macro_export]
#[doc(hidden)]
macro_rules! define_cast {
    ...
    (unsafe { $vis:vis $name:ident $(<$tyvar:ident $(: ?$optbound:ident)?>)? = $src:ty => $dst:ty }) => {
        ...
        // provenance.
        unsafe impl $(<$tyvar $(: ?$optbound)?>)? $crate::zerocopy::pointer::cast::Project<$src, $dst> for $name {
            fn project(src: $crate::zerocopy::pointer::PtrInner<'_, $src>) -> *mut $dst {
                #[allow(clippy::as_conversions)]
        ...
        unsafe impl $(<$tyvar $(: ?$optbound)?>)? $crate::zerocopy::pointer::cast::Cast<$src, $dst> for $name {}
    };
}
...
macro_rules! unsafe_impl_for_transparent_wrapper {
    ($vis:vis T $(: ?$optbound:ident)? => $wrapper:ident<T>) => {{
        ...
        use crate::zerocopy::pointer::{TransmuteFrom, cast::{CastExact, TransitiveProject}, SizeEq, invariant::Valid};
        use crate::zerocopy::wrappers::ReadOnly;
...
macro_rules! impl_transitive_transmute_from {
    ($($tyvar:ident $(: ?$optbound:ident)?)? => $t:ty => $u:ty => $v:ty) => {
        const _: () = {
            use crate::zerocopy::pointer::{TransmuteFrom, SizeEq, invariant::Valid};
