...
impl<L: Level, T> Mutex<L, T> {
    ...
    pub const fn new(val: T) -> Self {
        Self {
            inner: crate::spin::Mutex::new(val),
...
pub struct MutexGuard<'a, L: Level, T: ?Sized + 'a> {
    inner: crate::spin::MutexGuard<'a, T>,
...
impl<'a, L: Level, T: ?Sized + 'a> MutexGuard<'a, L, T> {
    ...
    /// the second a [`LockToken`] that can be used for further locking
    pub fn into_split(self) -> (crate::spin::MutexGuard<'a, T>, LockToken<'a, L>) {
...
impl<L: Level, T> RwLock<L, T> {
    ...
    pub const fn new(val: T) -> Self {
        Self {
            inner: crate::spin::RwLock::new(val),
            _phantom: PhantomData,
        }
    }
...
pub struct RwLockWriteGuard<'a, L: Level, T> {
    inner: crate::spin::RwLockWriteGuard<'a, T>,
    lock_token: LockToken<'a, L>,
}
...
pub struct RwLockReadGuard<'a, L: Level, T> {
    inner: crate::spin::RwLockReadGuard<'a, T>,
    lock_token: LockToken<'a, L>,
}
...
pub struct RwLockUpgradableGuard<'a, L: Level, T> {
    inner: crate::spin::RwLockUpgradableGuard<'a, T>,
    lock_token: LockToken<'a, L>,
}
...
impl<'a, L: Level, T> RwLockUpgradableGuard<'a, L, T> {
    ...
    pub fn upgrade(self) -> RwLockWriteGuard<'a, L, T> {
        RwLockWriteGuard {
            inner: crate::spin::RwLockUpgradableGuard::upgrade(self.inner),
            lock_token: self.lock_token,
        }
    }
