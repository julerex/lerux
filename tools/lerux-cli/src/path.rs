...
pub fn host_path(root: &Path) -> String {
    ...
    if !command_on_path("aarch64-none-elf-gcc")
        && let Ok(bin) = install_arm_toolchain(root)
    {
        paths.push(bin);
    }

    if !command_on_path("qemu-system-aarch64")
        && let Ok(bin) = install_qemu_aarch64(root)
    {
        paths.push(bin);
    }

    if !command_on_path("qemu-system-riscv64")
        && let Ok(bin) = install_qemu_riscv64(root)
    {
        paths.push(bin);
    }

    if !command_on_path("riscv64-unknown-elf-gcc")
        && let Ok(bin) = install_riscv_toolchain(root)
    {
        paths.push(bin);
    }
