...
use lerux_filesystem::{BlockAddr, BlockMeta, Disk, DiskFile, FileSystem};
use uuid::Uuid;
...
fn resize<D: Disk>(fs: &mut FileSystem<D>, size_arg: String) -> Result<(), String> {
    ...
    let min_size = if let Some(entry) = last_free {
        entry.index() * lerux_filesystem::BLOCK_SIZE
    } else {
        old_size
    };
    let max_size = disk_size - (fs.block * lerux_filesystem::BLOCK_SIZE);
    ...
    let old_blocks = old_size / lerux_filesystem::BLOCK_SIZE;
    let new_blocks = new_size / lerux_filesystem::BLOCK_SIZE;
...
fn main() {
    ...
    let free = fs.allocator().free() * lerux_filesystem::BLOCK_SIZE;
    let used = size - free;
