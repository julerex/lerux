...
fn block_addr_roundtrip() {
    let meta = BlockMeta {
        level: BlockLevel(3),
        decomp_level: Some(BlockLevel(1)),
    };
    let addr = unsafe { BlockAddr::new(42, meta) };
    assert_eq!(addr.index(), 42);
    assert_eq!(addr.level().0, 3);
    assert_eq!(addr.decomp_level().unwrap().0, 1);
    assert_eq!(addr.meta(), meta);
}

#[test]
fn block_addr_null_is_null() {
    let addr = BlockAddr::null(BlockMeta::default());
    assert!(addr.is_null());
}
