...
mod tests {
    use super::*;
    use crate::test_helpers::{temp_image_path, with_memory_fs};
    use crate::{DiskFile, DiskMemory, DiskSparse, SIGNATURE};
    use std::fs;

    #[test]
    fn create_sets_valid_header() {
        with_memory_fs(64 * 1024 * 1024, |fs| {
            assert!(fs.header.valid());
            assert_eq!(&fs.header.signature, SIGNATURE);
        });
    }

    #[test]
    fn create_fails_on_tiny_disk() {
        let disk = DiskMemory::new(BLOCK_SIZE * 2);
        assert!(FileSystem::create(disk, None, 1, 0).is_err());
    }

    #[test]
    fn open_fails_on_empty_disk() {
        let disk = DiskMemory::new(BLOCK_SIZE * 64);
        assert!(FileSystem::open(disk, None, None, false).is_err());
    }

    #[test]
    fn reopen_after_create_on_disk_file() {
        let path = temp_image_path();
        {
            let disk = DiskSparse::create(&path, 64 * 1024 * 1024).unwrap();
            FileSystem::create(disk, None, 100, 0).unwrap();
        }
        let disk = DiskFile::open(&path).unwrap();
        let fs = FileSystem::open(disk, None, None, true).unwrap();
        assert!(fs.header.valid());
        let _ = fs::remove_file(&path);
    }

    #[test]
    fn encrypted_create_requires_password_to_open() {
        let path = temp_image_path();
        {
            let disk = DiskSparse::create(&path, 64 * 1024 * 1024).unwrap();
            FileSystem::create(disk, Some(b"secret"), 100, 0).unwrap();
        }
        let disk = DiskFile::open(&path).unwrap();
        assert!(FileSystem::open(disk, None, None, false).is_err());
        let disk = DiskFile::open(&path).unwrap();
        assert!(FileSystem::open(disk, Some(b"wrong"), None, false).is_err());
        let disk = DiskFile::open(&path).unwrap();
        assert!(FileSystem::open(disk, Some(b"secret"), None, true).is_ok());
        let _ = fs::remove_file(&path);
    }
}
