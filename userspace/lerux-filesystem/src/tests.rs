...
#[test]
fn golden_image_write_reopen_read() {
    use crate::test_helpers::temp_image_path;
    use crate::{DiskFile, DiskSparse, Node, TreePtr};
    use std::fs;

    let path = temp_image_path();
    let payload = b"lerux-golden-persist";
    {
        ...
        let file = fs
            ...
            .unwrap();
        fs.tx(|tx| tx.write_node(file.ptr(), 0, payload, ctime.as_secs(), ctime.subsec_nanos()))
            .unwrap();
    }
    let disk = DiskFile::open(&path).unwrap();
    let mut fs = FileSystem::open(disk, None, None, true).unwrap();
    let root = TreePtr::<Node>::root();
    let file = fs.tx(|tx| tx.find_node(root, "golden.txt")).unwrap();
    let mut buf = [0u8; 32];
    let n = fs.tx(|tx| tx.read_node(file.ptr(), 0, &mut buf, 1, 0)).unwrap();
...
fn rename_preserves_file_contents() {
    with_redoxfs(|mut fs| {
        let root = TreePtr::<Node>::root();
        let file = fs
            .tx(|tx| tx.create_node(root, "before", Node::MODE_FILE | 0o644, 1, 0))
            .unwrap();
        let data = b"rename-me";
        fs.tx(|tx| tx.write_node(file.ptr(), 0, data, 2, 0)).unwrap();
        fs.tx(|tx| tx.rename_node(root, "before", root, "after"))
            .unwrap();
        let renamed = fs.tx(|tx| tx.find_node(root, "after")).unwrap();
        let mut buf = [0u8; 16];
        let n = fs
            .tx(|tx| tx.read_node(renamed.ptr(), 0, &mut buf, 3, 0))
            .unwrap();
