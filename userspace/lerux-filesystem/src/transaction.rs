...
mod transaction_tests {
    use crate::test_helpers::with_memory_fs;
    use crate::{Node, TreePtr};
    use syscall::error::{EEXIST, ENOENT};

    #[test]
    fn create_find_and_read_file() {
        with_memory_fs(64 * 1024 * 1024, |mut fs| {
            let root = TreePtr::<Node>::root();
            let file = fs
                .tx(|tx| {
                    tx.create_node(root, "hello.txt", Node::MODE_FILE | 0o644, 1, 0)
                })
                .unwrap();
            let data = b"hello lerux";
            fs.tx(|tx| tx.write_node(file.ptr(), 0, data, 2, 0)).unwrap();
            let mut buf = [0u8; 32];
            let n = fs
                .tx(|tx| tx.read_node(file.ptr(), 0, &mut buf, 3, 0))
                .unwrap();
    ...
    fn nested_directories() {
        with_memory_fs(64 * 1024 * 1024, |mut fs| {
            ...
            let sub = fs
                .tx(|tx| {
                    tx.create_node(root, "a", Node::MODE_DIR | 0o755, 1, 0)
                })
                .unwrap();
            let inner = fs
                .tx(|tx| {
                    tx.create_node(sub.ptr(), "b", Node::MODE_DIR | 0o755, 1, 0)
                })
                .unwrap();
            fs.tx(|tx| {
                tx.create_node(inner.ptr(), "c.txt", Node::MODE_FILE | 0o644, 1, 0)
            })
            .unwrap();
            fs.tx(|tx| tx.find_node(inner.ptr(), "c.txt")).unwrap();
        });
    }

    #[test]
    fn rename_node_moves_file() {
        with_memory_fs(64 * 1024 * 1024, |mut fs| {
            let root = TreePtr::<Node>::root();
            fs.tx(|tx| {
                tx.create_node(root, "old.txt", Node::MODE_FILE | 0o644, 1, 0)
            })
            .unwrap();
            fs.tx(|tx| tx.rename_node(root, "old.txt", root, "new.txt"))
                .unwrap();
            fs.tx(|tx| tx.find_node(root, "new.txt")).unwrap();
            assert!(fs.tx(|tx| tx.find_node(root, "old.txt")).is_err());
        });
    }
    ...
    fn rename_no_replace_fails_if_target_exists() {
        with_memory_fs(64 * 1024 * 1024, |mut fs| {
            let root = TreePtr::<Node>::root();
            fs.tx(|tx| {
                tx.create_node(root, "a.txt", Node::MODE_FILE | 0o644, 1, 0)
            })
            .unwrap();
            fs.tx(|tx| {
                tx.create_node(root, "b.txt", Node::MODE_FILE | 0o644, 1, 0)
            })
            .unwrap();
            let err = fs
                .tx(|tx| tx.rename_node_no_replace(root, "a.txt", root, "b.txt"))
                .unwrap_err();
    ...
    #[test]
    fn remove_missing_file_returns_enoent() {
        with_memory_fs(64 * 1024 * 1024, |mut fs| {
            let root = TreePtr::<Node>::root();
            let err = fs
                .tx(|tx| tx.remove_node(root, "missing", Node::MODE_FILE))
                .unwrap_err();
    ...
    fn truncate_extends_file_with_zeroes() {
        with_memory_fs(64 * 1024 * 1024, |mut fs| {
            ...
            let file = fs
                .tx(|tx| {
                    tx.create_node(root, "grow.txt", Node::MODE_FILE | 0o644, 1, 0)
                })
                .unwrap();
            fs.tx(|tx| tx.write_node(file.ptr(), 0, b"x", 1, 0)).unwrap();
            fs.tx(|tx| tx.truncate_node(file.ptr(), 4096, 2, 0)).unwrap();
            let size = fs.tx(|tx| tx.read_tree(file.ptr())).unwrap().data().size();
            assert_eq!(size, 4096);
        });
    }
}
